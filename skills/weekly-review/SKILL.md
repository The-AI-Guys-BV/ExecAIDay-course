...
- Confirm vault mounted with GTD files and Daily Notes.
...
Read Daily Notes from past 7 days in `<vault>/Daily Notes/`. Surface 3–5 accomplishments.

Wait. User confirms or adds. Move on.

## Step 2 — projects without next actions

Read `<vault>/gtd/projects/` files. For each project, check `<vault>/gtd/next-actions.md` for bullets tagged `@project:<slug>`.

Present each one. For each, ask: "What's the next concrete action?" Update `<vault>/gtd/projects/<slug>.md` with the user's answer.
...
Read `<vault>/gtd/waiting-for.md`. Filter: items waited-on > 7 days.

Present each. For each, offer:
- Follow up (drafts a follow-up email via `/drafts-inbox`)
- Push deadline (update the date in vault project file)
...
Read `<vault>/gtd/next-actions.md`. Filter: created > 14 days ago AND Status = Active.
...
- Don't make changes to vault without the user explicitly choosing the action.
...
1. **Vault not set up** → tell user; offer to walk just the calendar + Daily Notes portions.
2. **GTD files don't match expected schema** → list what's missing; fall back to read-only review without writes.
3. **No Daily Notes for the week** → skip Step 1, note it.
4. **User wants to abort mid-review** → save state in `weekly-review-state.md`, allow resume.
